...
grid = "g1"
figPath = f"/squall/gleung/borneolcc-figures/satmod-{lcver}-{rver}/"
...
def plot_quicklook(ds, time, ahipaths, glon, glat, savePath):
    ...
    scn = scn.resample(scn["B03"].attrs["area"], resampler="nearest")
    scn = scn.crop(ll_bbox=(108.65, -0.164, 111.61, 2.85))
    scn = scn.resample(scn["B03"].attrs["area"], resampler="nearest")
    scn.load(["geo_color"])
    crs = scn["geo_color"].attrs["area"].to_cartopy_crs()
    a = scn.show("geo_color")
    ...
    axes[0].imshow(
        a.pil_image(), transform=crs, extent=crs.bounds, origin="upper"
    )
    ...
    axes[0].set_title("(a) AHI GeoColor")
...
for paths in np.array_split(sorted(all_paths), len(all_paths) // 15):
    ...
    ds = client.map(
        get_rams_output,
        [p for p in paths],
        variables=["WP", "RCP", "RSP", "RPP", "PCPRR"],
        prep_tobac=True,
    )
    ...
    out = client.map(
        ...
        savePath=f"/squall/gleung/borneolcc-figures/satmod-{lcver}-{rver}/",
    )
