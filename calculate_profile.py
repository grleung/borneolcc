...
mean thermodynamic profiles over land points only as an average over the local time 
diurnal cycle (over the three simulation days). By default,
the time resolution is 30mins. There is one hdf5 file per run, which are saved under
 '/squall/gleung/borneolcc-analysis/paper-analysis/profile-thermo-diurnal-[run].h5'
"""
...
times = np.arange(0, 24, time_resolution)  # times to check as hour of the day (localtime)
...
for run in runs:
    ...
    for t in times:
        ...
        ds = client.map(compute_thermo_prof, ds)

        # compute mean over land points
        ds = client.map(get_land_mean, ds, landmask=landmask, profile=True)
    ...
    out.to_netcdf(
        f"/squall/gleung/borneolcc-analysis/paper-analysis/prof-thermo-diurnal-{run}.h5",
        engine="h5netcdf"
    )
